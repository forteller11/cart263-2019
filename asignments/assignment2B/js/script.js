...
to overlap other anothers circle (food) in order to grow bigger.
(also the foods move a little bit (with the help of perlin-noise!))
...
function setup() {
  ...
  for (let i = 0; i < foodPopulation; i ++){
    food[i] = new Food();
  }
  noCursor();
}
...
function checkCollision() {
  ...
  for (let i = 0; i < food.length; i ++){
    let d = dist(avatar.x,avatar.y,food[i].x,food[i].y);
    if (d < avatar.size/2 + food[i].size/2) {
      avatar.size = constrain(avatar.size + AVATAR_SIZE_GAIN,0,avatar.maxSize);
      food[i].x = random(width); //NEW
      food[i].y = random(height); //NEW
    }
  }
